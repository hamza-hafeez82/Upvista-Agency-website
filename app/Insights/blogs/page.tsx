    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Link href="/Services/web-development">
              <button className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg font-medium transition-all flex items-center justify-center">
                {t('blogs.hero.cta.primary')} <ArrowRight className="ml-2" size={18} />
              </button>
            </Link>
            <Link href="/Services/software-development">
              <button className="border border-blue-600 hover:bg-blue-50 text-blue-600 px-8 py-3 rounded-lg font-medium transition-all">
                {t('blogs.hero.cta.secondary')}
              </button>
            </Link>
          </div>
        </div>
      ...
      <section className="py-16 px-4 md:px-8 max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <span className="text-blue-600 font-medium">{t('blogs.research.badge')}</span>
          <h2 className="text-3xl md:text-4xl font-bold mt-2 mb-4 text-gray-900">
            {t('blogs.research.title')}
          </h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            {t('blogs.research.subtitle')}
          </p>
        </div>

        <div className="space-y-8">
          {researchPapers.map((paper, index) => (
            <article key={index} className="bg-white rounded-xl overflow-hidden hover:transform hover:scale-[1.02] transition-all duration-300 border border-gray-200 shadow-lg">
              <div className="p-8">
                      ...
                      <ul className="space-y-2">
                        {paper.keyFindings.map((finding, idx) => (
                          <li key={idx} className="flex items-start">
                            <Target className="w-4 h-4 text-blue-600 mr-3 mt-0.5 flex-shrink-0" />
      ...
      <section className="py-16 px-4 md:px-8 max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <span className="text-blue-600 font-medium">{t('blogs.services.badge')}</span>
          <h2 className="text-3xl md:text-4xl font-bold mt-2 mb-4 text-gray-900">
            {t('blogs.services.title')}
          </h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            {t('blogs.services.subtitle')}
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Service Navigation */}
          <div className="bg-white rounded-xl p-6 h-fit border border-gray-200 shadow-lg">
            <h3 className="text-xl font-semibold mb-4 text-gray-900">
              {t('blogs.services.researchAreas')}
            </h3>
            <ul>
              {services.map((service, index) => (
                <li key={index}>
                  ...
                  >
                    <span>{t(`blogs.services.${service.title.toLowerCase().replace(/\s+/g, '')}`)}</span>
                    <ChevronRight
                      size={18}
                      className={
                        activeService === index
          ...
          {/* Service Details */}
          <div className="md:col-span-2 bg-white rounded-xl p-8 border border-gray-200 shadow-lg">
            <div className="mb-6">
              {services[activeService].icon}
              <h3 className="text-2xl font-bold text-gray-900 mb-4">
                {t(`blogs.services.${services[activeService].title.toLowerCase().replace(/\s+/g, '')}`)}
              </h3>
              <p className="text-gray-600 mb-6">
                {t(`blogs.services.${services[activeService].title.toLowerCase().replace(/\s+/g, '')}Description`)}
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div>
                <h4 className="text-lg font-semibold text-gray-900 mb-4">{t('blogs.services.advantages')}</h4>
                ...
                {services[activeService].advantages.map((advantage, index) => (
                  <li key={index} className="flex items-start">
                    ...
                    <span className="text-gray-600">{t(`blogs.services.${services[activeService].title.toLowerCase().replace(/\s+/g, '')}Advantage${index + 1}`)}</span>
                  </li>
                ))}
              </ul>
            </div>
              <div>
                <h4 className="text-lg font-semibold text-gray-900 mb-4">{t('blogs.services.evidence')}</h4>
      ...
      <section className="py-16 px-4 md:px-8 max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <span className="text-blue-600 font-medium">{t('blogs.research.domains')}</span>
            <h2 className="text-3xl md:text-4xl font-bold mt-2 mb-4 text-gray-900">
            {t('blogs.research.areas')}
            </h2>
            <p className="text-gray-600 max-w-2xl mx-auto">
            {t('blogs.research.description')}
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white p-6 rounded-xl text-center group hover:bg-gray-50 transition-all border border-gray-200 shadow-lg">
            <Cpu className="w-12 h-12 text-blue-600 mx-auto mb-4 group-hover:scale-110 transition-transform" />
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('blogs.research.ai')}</h3>
            <p className="text-gray-600 text-sm">
              {t('blogs.research.aiDescription')}
            </p>
          </div>
          <div className="bg-white p-6 rounded-xl text-center group hover:bg-gray-50 transition-all border border-gray-200 shadow-lg">
            <Network className="w-12 h-12 text-blue-600 mx-auto mb-4 group-hover:scale-110 transition-transform" />
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('blogs.research.blockchain')}</h3>
            <p className="text-gray-600 text-sm">
              {t('blogs.research.blockchainDescription')}
              </p>
            </div>
          <div className="bg-white p-6 rounded-xl text-center group hover:bg-gray-50 transition-all border border-gray-200 shadow-lg">
            <Brain className="w-12 h-12 text-blue-600 mx-auto mb-4 group-hover:scale-110 transition-transform" />
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('blogs.research.neuroscience')}</h3>
            <p className="text-gray-600 text-sm">
              {t('blogs.research.neuroscienceDescription')}
              </p>
            </div>
          <div className="bg-white p-6 rounded-xl text-center group hover:bg-gray-50 transition-all border border-gray-200 shadow-lg">
            <Shield className="w-12 h-12 text-blue-600 mx-auto mb-4 group-hover:scale-110 transition-transform" />
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('blogs.research.cybersecurity')}</h3>
            <p className="text-gray-600 text-sm">
              {t('blogs.research.cybersecurityDescription')}
              </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-4 md:px-8 max-w-6xl mx-auto">
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link href="/Services/software-development">
              <button className="border border-white text-white px-8 py-3 rounded-lg font-medium hover:bg-white hover:text-blue-600 transition-all">
                {t('blogs.cta.secondary')}
              </button>
            </Link>
          </div>
        </div>
